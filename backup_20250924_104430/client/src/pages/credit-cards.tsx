...
export default function CreditCards() {
  ...
  const getOverdueCredits = () => {
    ...
    return credits.filter((credit: Credit) => {
      ...
      return dueDate < today && credit.amount && parseFloat(credit.amount) > 0;
    });
  };
  ...
  const totalMinimumPayments = credits
    ...
    .reduce((sum: number, credit: Credit) => sum + parseFloat(credit.minimumPayment), 0);
            ...
            <Table>
              ...
              <TableBody>
                {credits.map((credit: Credit) => {
                  const isOverdue = new Date(credit.dueDate) < new Date() && parseFloat(credit.amount || "0") > 0;
                  return (
                    <TableRow key={credit.id} className={isOverdue ? "bg-red-50" : ""}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          {new Date(credit.dueDate).toLocaleDateString("tr-TR")}
                        </div>
                      </TableCell>
                      <TableCell className="text-right">
