...
export async function authenticatedApiRequest(
  ...
  let response = await apiRequest(method, url, headers, body);

  // If token is expired (401), try to refresh
  if (response.status === 401 && accessToken) {
    const refreshToken = localStorage.getItem('refreshToken');
    if (refreshToken) {
      const refreshResponse = await apiRequest('POST', '/api/auth/jwt/refresh', {
        'Content-Type': 'application/json'
      ...
      if (refreshResponse.ok) {
        ...
        response = await apiRequest(method, url, headers, body);
      }
    }
  }
