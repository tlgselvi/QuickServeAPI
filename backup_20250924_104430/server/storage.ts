...
export class MemStorage implements IStorage {
  ...
  async processRecurringExpenses(): Promise<{ processed: number; transactions: Transaction[] }> {
    ...
    for (const expense of this.fixedExpenses.values()) {
  ...
  async createForecast(forecastData: InsertForecast): Promise<Forecast> {
    const forecast: Forecast = {
      ...
      predictedValue: forecastData.predictedValue,
...
export class PostgresStorage implements IStorage {
  ...
  async getTransactionsPaginated(page: number, limit: number, search?: string, accountId?: string): Promise<{ transactions: Transaction[]; total: number; totalPages: number }> {
    ...
    if (accountId) {
      ...
      countQuery = countQuery.where(eq(transactions.accountId, accountId));
    }

    if (search) {
      const searchCondition = sql`${transactions.description} ILIKE ${`%${search}%`} OR ${transactions.category} ILIKE ${`%${search}%`} OR ${transactions.amount}::text ILIKE ${`%${search}%`}`;
  ...
  async getFixedExpenses(): Promise<FixedExpense[]> {
    return await db.select().from(fixedExpenses).orderBy(desc(fixedExpenses.createdAt));
  }
  ...
  async getCredits(): Promise<Credit[]> {
    return await db.select().from(credits).orderBy(desc(credits.createdAt));
  }
  ...
  async getActiveForecasts(): Promise<Forecast[]> {
    return await db.select().from(forecasts).where(eq(forecasts.isActive, true));
  }

  // AI Settings methods
  async getAISettings(): Promise<any> {
    // Return the first (and only) AI settings record
    for (const [_, settings] of this.aiSettings) {
      return settings;
    }
    return null;
  }
