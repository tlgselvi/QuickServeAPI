...
router.get('/', requireRole('admin'), async (req: Request, res: Response) => {
  try {
    const tenants = MemStorage.getTenants();
    res.json(tenants);
  } catch (error) {
...
router.get('/:id', requireRole('admin'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const tenant = MemStorage.getTenantById(id);
...
router.post('/', requireRole('admin'), async (req: Request, res: Response) => {
  try {
    ...
    const tenant = MemStorage.createTenant(validatedData);
...
router.put('/:id', requireRole('admin'), async (req: Request, res: Response) => {
  try {
    ...
    const tenant = MemStorage.updateTenant(id, validatedData);
...
router.delete('/:id', requireRole('admin'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const success = MemStorage.deleteTenant(id);
...
router.get('/domain/:domain', async (req: Request, res: Response) => {
  try {
    ...
    const tenant = MemStorage.getTenantByDomain(domain);
