...
import { requireJWTAuth, requireJWTPermission, requireJWTAdmin, logJWTAccess, AuthenticatedRequest as JWTAuthenticatedRequest } from "./middleware/jwt-auth";
  ...
  app.get("/api/scenarios/predefined",
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        const scenarios = ScenarioAnalysisService.getPredefinedScenarios();
        res.json(scenarios);
      } catch (error) {
  ...
  app.post("/api/scenarios/analyze",
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        const { ScenarioAnalysisService } = await import('./scenario-analysis-service');
        const result = await ScenarioAnalysisService.analyzeScenario(scenarioName, parameters);
  ...
  app.post("/api/scenarios/analyze/:scenarioKey",
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        const { ScenarioAnalysisService } = await import('./scenario-analysis-service');
        const predefinedScenarios = ScenarioAnalysisService.getPredefinedScenarios();
        
        const scenario = predefinedScenarios.find(s => 
          s.name.toLowerCase().replace(/\s+/g, '_') === scenarioKey
        );
        ...
        const result = await ScenarioAnalysisService.analyzeScenario(
          scenario.name, 
          scenario.parameters
        );
